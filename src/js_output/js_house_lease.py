...
def lease_output():
    ...
    query = """ SELECT list_price, latitude, longitude, mls, property_type, region, address,
        beds, bath, sf, zip, case current when 1 then 1 else 0 end as "current"
        FROM austin_lease WHERE geocode_status = 'OK' and property_type = 'HOUSE' and zip is not null """
    cur.execute(query)
    recs = cur.fetchall()
    cur.close()
    con.close()
